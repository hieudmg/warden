...
describe("DBForm", () => {
  ...
  test("adds rows, changes the default, and removes a row", async () => {
    ...
    expect(within(table).getByRole("columnheader", { name: "Default" })).toBeInTheDocument()
  ...
  test("blocks submission when database names are duplicated", async () => {
    ...
    await user.click(screen.getByRole("button", { name: "Add database" }))
    await user.type(screen.getAllByRole("textbox", { name: "Database name" })[0], "main")
    await user.type(screen.getAllByRole("textbox", { name: "Database name" })[1], "main")
    await user.click(screen.getByRole("button", { name: "Save" }))
  ...
  test("selecting an existing profile maps to its numeric ID", async () => {
    ...
    await user.type(screen.getByLabelText("Host"), "127.0.0.1")
    await user.type(screen.getByLabelText("Username"), "app")
    await user.type(screen.getByLabelText("Database name"), "warden")
    await user.click(screen.getByRole("combobox", { name: "SSH connection" }))
